...
impl fmt::Display for CommitType {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            CommitType::Style => write!(
                f,
                "\u{1F60E} style: Styling changes (no change to actual meaning of the code"
            ),
            ...
            CommitType::Test => {
                write!(f, "\u{1F3E5} test: Adding test or correcting existing test")
            }
...
fn get_input_value_req(input: &str, msg: &str) -> String {
    ...
    match prompt_val {
        Ok(prompt_val) =>  prompt_val,
        Err(_) => panic!(
            "An error happened when asking for {}, try again later.",
            input
        ),
    }
}
...
fn get_input_value(input: &str, msg: &str) -> String {
    ...
    match prompt_val {
        Ok(prompt_val) => {
            if input == "breaking_change" && !prompt_val.is_empty() {
...
fn get_ctype_flag(ctype: CommitType) -> &'static str {
    match ctype {
        CommitType::Fix => "fix",
...
fn main() {
    ...
    let sdesc = get_input_value_req(
        "short_description",
        "Write short imperative tense description of the change?",
    );
    let ldesc = get_input_value_req(
        "long_description",
        "Provide a longer description of the change?",
    );
    let break_change = get_input_value(
        "breaking_change",
